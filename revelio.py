...
Description:
	...
	query and reference sequences. The script requires either an input FASTA 
	sequence of the reference genome (slow) or a BAM file with MD tags (fast) 
	as generated by 'samtools calmd'.
...
Procedure:
	...
	5. Use DoubleMasking procedure to mask bisulfite-seq information
...
eg. ./revelio.py -f reference.fa -Q -T 1 -t /var/tmp input.bam output.bam
eg. ./revelio.py input.bam output.bam
'''
	...
	with pysam.AlignmentFile(BAM, "rb") as original, pysam.AlignmentFile(name, "wb", header=header) as modified:
		...
		for alignment in original.fetch(ref):
			...
			if alignment.flag == 16:
				CT = False
			elif alignment.flag == 0:
				CT = True
			else:
				raise Exception(f"Unexpected flag mask: {alignment.flag}")
...
# define argparse
usage = ''' This program takes an input BAM file and masks base positions that are 
	potentially influenced by bisulfite-seq as observed from alignments between 
	query and reference sequences. The script requires either an input FASTA 
	sequence of the reference genome (slow) or a BAM file with MD tags (fast) 
	as generated by 'samtools calmd'. '''
